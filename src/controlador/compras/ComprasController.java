...
//        x.initModality(Modality.APPLICATION_MODAL);
    ...
    public void ini2() {
        ...
        TableColumn<modelo_venta, String> tc_precio = tprecio;
        tc_precio.setCellValueFactory(new PropertyValueFactory<modelo_venta, String>("precio"));
        tc_precio.setCellFactory(TextFieldTableCell.forTableColumn());
        tc_precio.setOnEditCommit(data -> {
            modelo_venta p = data.getRowValue();
            if (!data.getNewValue().matches("[0-9]+(\\.[0-9][0-9])?")) {
